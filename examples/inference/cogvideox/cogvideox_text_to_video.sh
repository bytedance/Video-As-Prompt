...
if [ ! -d "data/videoanimator/openvid-1k-split-validation" ]; then
  echo "Downloading validation dataset..."
  huggingface-cli download --repo-type dataset finetrainers/OpenVid-1k-split-validation --local-dir data/videoanimator/openvid-1k-split-validation
else
...
model_cmd=(
  ...
  --pretrained_model_name_or_path "ckpts/CogVideoX-5b"
  --enable_slicing
  --enable_tiling
)
...
miscellaneous_cmd=(
  ...
  --output_dir "outputs_finfer/debug_cogvideo"
  --runs_name "debug_cogvideo"
  --init_timeout 600
  --nccl_timeout 600
  --report_to "wandb"
)
