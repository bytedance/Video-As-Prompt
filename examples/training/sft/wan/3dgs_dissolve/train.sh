...
if [ ! -d "data/finetrainers-3dgs-dissolve/validation_dataset" ]; then
  echo "Downloading validation dataset..."
  huggingface-cli download --repo-type dataset finetrainers/OpenVid-1k-split-validation --local-dir data/finetrainers-3dgs-dissolve/validation_dataset
else
...
model_cmd=(
  --model_name "wan"
  --pretrained_model_name_or_path "ckpt/Wan2.1-I2V-14B-480P-Diffusers"
...
miscellaneous_cmd=(
  --output_dir "outputs/2000-wan-i2v-3dgs-dissolve"
  --runs_name "2000-wan-i2v-3dgs-dissolve"
  --init_timeout 600
  --nccl_timeout 600
  --report_to "wandb"
)
