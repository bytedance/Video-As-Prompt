...
def nearst_resize_video(video: torch.Tensor, size: Tuple[int, int]) -> torch.Tensor:
    num_frames, num_channels, height, width = video.shape
    video = F.interpolate(video, size=size, mode="nearest")
    return video
...
def resize_to_nearest_bucket_video(
    ...
    if num_frames > target_frames:
        if frames_selection == "evenly":
            # Downsample: Select frames evenly
            indices = torch.linspace(0, num_frames - 1, target_frames).long()
            video = video[indices, :, :, :]
        elif frames_selection == "first":
            # Downsample: Select first frames
            video = video[:target_frames, :, :, :]
        elif frames_selection == "random":
            start_index = random.randint(0, num_frames - target_frames)
            indices = torch.arange(start_index, start_index + target_frames)
            video = video[indices, :, :, :]
            # print(f"indices: {indices}")
        else:
            raise ValueError(f"Invalid frames_selection: {frames_selection}. Choose from 'first' or 'evenly'.")
    ...
    if resize_mode == "center_crop":
        ...
        return nearst_resize_video(video, (target_h, target_w)), _first_frame_only
    else:
        raise ValueError(
